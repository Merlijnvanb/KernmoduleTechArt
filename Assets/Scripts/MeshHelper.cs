...
//MeshHelper gotten from user Bunny83 on pastebin
public static class MeshHelper
{
    ...
    static Dictionary<(int, int), int> newVertices;
    ...
    static int GetNewVertex4(int i1, int i2)
    {
        ...
        if (newVertices.TryGetValue(key, out int existingIndex))
    ...
    {
        newVertices = new Dictionary<(int, int), int>();
    ...
    static int GetNewVertex9(int i1, int i2, int i3)
    {
        ...
        // Only edge points go in the dictionary
        if (i3 == i1 || i3 == i2)
        {
            var key = i1 < i2 ? (i1, i2) : (i2, i1);
            if (newVertices.TryGetValue(key, out int existingIndex))
